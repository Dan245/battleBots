 ...
 * @Version <br>
 *          1.13 (Oct 26, 2022) - Brooks - Add team scoring option
 * @author Sam Scott
...
public class BattleBotArena extends JPanel
	...
	public static final int BOTTOM_EDGE = 600; // ROWBOTTOM - changed from 500// arena panel height is this constant +
												// TEXT_BUFFER
	...
	private void fullReset() {
		...
		// bots[4] = new KimBot();
		bots[4] = new RandBot();
		bots[5] = new SlyzysBot();
		bots[6] = new MamoBot();
		bots[7] = new RisticBot();
		bots[8] = new ModhvadiaBot();
		bots[9] = new ForbesBot();
		bots[10] = new NorthcottBot();
		bots[11] = new JackBot();
		bots[12] = new HreceniukBot();
		bots[13] = new RandBot();
		// bots[13] = new HughesBot();
		bots[14] = new MacDonaldBot();
		bots[15] = new ZhangBot();
		bots[16] = new CakirBot();
		bots[17] = new RamosBot();
		bots[18] = new JiangBot();
		bots[19] = new Qiao_Bot();
		// bots[20] = new SalimBot();
		bots[20] = new RandBot();
		bots[21] = new AtassiBot();
		bots[22] = new RandBot();
		// bots[22] = new BatthBot();
		bots[23] = new YassinBot();
		bots[24] = new RayanBot();
			...
			if (state != TEST_MODE && (timePlayed >= TIME_LIMIT || botsLeft <= 1)) {
				...
				if (CUMULATIVE_SCORING && TEAM_SCORING) {
					...
					for (int i = 0; i < NUM_BOTS; i++) {
						...
						if (score != null) {// team not in map, add to map and add score
							...
							score = botsInfo[i].getScore();
							// System.out.println("Setting initial score for " + bots[i].getTeamName() + "
							// to "+ botsInfo[i].getScore() );
				...
				if (totalOut >= NUM_BOTS - 1) // is this the last round?
				{
					...
					if (CUMULATIVE_SCORING) { // different message depending on scoring type
						if (TEAM_SCORING)
							sendMessage(-1,
									"Round " + round + " complete. " + sortedBots[0].getTeamName() + " is leading.");
						else
							sendMessage(-1,
									"Round " + round + " complete. " + sortedBots[0].getName() + " is leading.");
		...
		if (ready) // avoid race condition on startup
		{
			...
			if (state == GAME_PAUSED) {
				...
				if (CUMULATIVE_SCORING) {
					if (TEAM_SCORING)
						g.drawString(
								botsInfo[winnerID].getTeamName() + " Leading After " + round + " Round"
										+ (round > 1 ? "s" : "") + ". Click This Bar to Continue.",
								10, BOTTOM_EDGE - 10);
					else
						g.drawString(
								botsInfo[winnerID].getName() + " Leading After " + round + " Round"
										+ (round > 1 ? "s" : "") + ". Click This Bar to Continue.",
								10, BOTTOM_EDGE - 10);
				} else
					g.drawString(
				...
				if (CUMULATIVE_SCORING)
					if (TEAM_SCORING)
						g.drawString(
								botsInfo[winnerID].getTeamName() + " Wins after " + round + " Round"
										+ (round > 1 ? "s" : "") + "! Click This Bar to Restart.",
								10, BOTTOM_EDGE - 10);
					else
						g.drawString(
								botsInfo[winnerID].getName() + " Wins after " + round + " Round"
										+ (round > 1 ? "s" : "") + "! Click This Bar to Restart.",
								10, BOTTOM_EDGE - 10);
				else
